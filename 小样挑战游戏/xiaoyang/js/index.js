...
$(document).ready(function (){
	$("<audio id='music-bg' loop src='' type='audio/mpeg'></audio>").appendTo($('#content'));
	$("<audio id='music-bomb' src='' type='audio/mpeg'></audio>").appendTo($('#content'));
	$("<audio id='music-countdown' src='' type='audio/mpeg'></audio>").appendTo($('#content'));
	$('#music-bg').attr('src','audio/bgm.mp3');
	$('#music-bomb').attr('src','audio/bomb.mp3');
	$('#music-countdown').attr('src','audio/lastfive2.mp3');
	//从后台获取数据
	//音频
	$('#music-bg').get(0).addEventListener('loadedmetadata',function () {
		$('#music-bg').get(0).autoplay = false;
		$('#music-bg').get(0).loop = true;
...
//
	$('#music-bomb').get(0).addEventListener('loadedmetadata',function () {
		...
		$('#music-bomb').get(0).pause();
	},false);
	$('#music-countdown').get(0).addEventListener('loadedmetadata',function () {
		$('#music-countdown').get(0).currentTime = 0;
		$('#music-countdown').get(0).pause();
	},false);
	...
	$('#music-countdown').get(0).addEventListener('ended',function () {
		playBgm ();
		$('#music-countdown').get(0).currentTime = 0;
	},false);
...
//获奖规则页面
	...
	$('#playbomb').get(0).addEventListener('click',playBomb,false);
	function playBomb () {
		$('#music-bomb').get(0).currentTime = 0;
		$('#music-bomb').get(0).play();
	}
	$('#playbgm').get(0).addEventListener('click',playBgm,false);
	function playBgm () {
		$('#music-bg').get(0).play();
	}
	$('#playcountdown').get(0).addEventListener('click',playCountdown,false);
